...
(function () {
  ...
  function login(username, password) {
    return fetchWithTimeout(baseUrl() + "/auth/login", {
      ...
      if (!parsed.token) {
        console.warn(
          "[bookStoreApi] Login 200 but no token in response:",
          data,
        );
      }
      return parsed;
  ...
  function getBooks() {
    return fetch(baseUrl() + "/books").then(async function (res) {
      const data = await parseBody(res);
      if (!res.ok) {
  ...
  function getBookById(id) {
    return fetch(baseUrl() + "/books/" + encodeURIComponent(id)).then(
      async function (res) {
  ...
  function getCart() {
    ...
    return fetch(baseUrl() + "/cart", { headers: authHeaders(t) }).then(
      async function (res) {
        const data = await parseBody(res);
        if (!res.ok) {
  ...
  function updateCartItem(bookId, quantity) {
    ...
    return fetch(baseUrl() + "/cart/" + encodeURIComponent(bookId), {
      ...
      const data = await parseBody(res);
      if (!res.ok)
        throw new Error(data.message || "به‌روزرسانی سبد ناموفق بود.");
      return data.cart;
    });
  }
  ...
  function listOrders() {
    const t = getToken();
    if (!t) return Promise.reject(new Error("نیاز به ورود"));
    return fetch(baseUrl() + "/orders", { headers: authHeaders(t) }).then(
      async function (res) {
        const data = await parseBody(res);
        if (!res.ok)
          throw new Error(data.message || "دریافت سفارش‌ها ناموفق بود.");
